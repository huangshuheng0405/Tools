...
| timestamp | YYYY-MM-DD HH:MM:SS | 1970-01-01 00:00:01~2038-01-19 03:14:07 | 4个字节  |

### 字符串

| 类型名称   | 说明             | 存储需求                          |
| ---------- | ---------------- | --------------------------------- |
| char(M)    | 固定长度字符串   | M个字符，最大255个字符，M字节     |
| varchar(M) | 可变长度字符串   | M个字符，最大65535个字符，L+1字节 |
| tinytext   | 微型文本         | 255字节                           |
| text       | 文本             | 65535字节（64KB）                 |
| mediumtext | 中等长文本       | 16777215字节（16MB）              |
| longtext   | 大文本数据       | 4294967295字节（4GB）             |
| enum       | 枚举（单选）     | 1~2个字节                         |
| set        | 集合（多选）     | 1~8个字节                         |
...
| varbinary(M)  | 可变长度二进制字符串 | L+1个字节                         |
| tinyblob(M)   | 非常小的BLOB         | 255字节                           |
| blob(M)       | 小BLOB               | 65535字节（64KB）                 |
| mediumblob(M) | 中等大小的BLOB       | 16777215字节（16MB）              |
| longblob(M)   | 非常大的BLOB         | 4294967295字节（4GB）             |
