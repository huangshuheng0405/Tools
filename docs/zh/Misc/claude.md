...
1. `resolve-library-id` — 把库名解析为 Context7 库 ID(用官方名称,如 "Next.js" 而非 "nextjs";提到版本时用带版本号的 ID)
2. `query-docs` — 用库 ID 查询文档,一个概念一次查询(多个概念分开查,合并会稀释结果)

适合: 库文档、API 语法、配置、版本迁移、库级调试、CLI 工具用法
不适合: 重构、从零写脚本、调试业务逻辑、代码审查、通用编程概念
