在 Vue Router 中，`query` 和 `params` 是两种最常用的传参方式。它们最直观的区别就像是 **GET 请求**与 **POST 请求**在地址栏表现上的差异。
...
| **刷新丢失** | **不会**丢失（参数在 URL 中）  | **会**丢失（如果不配合动态路由匹配）        |
...
> **注意（Vue Router 4.x 重要变更）：** > 在 Vue 3 中，如果你直接使用 `params` 传递内存中的对象且不在路径中定义它（即隐式传参），刷新页面后参数**会消失**。官方推荐使用动态路由匹配或状态管理（Pinia/Vuex）替代。
